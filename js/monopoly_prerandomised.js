...
var slotTypes = [
    ...
    {type: 'tax', characters: ['littlemac', 'miigunner'], pool: ['littlemac', 'drmario', 'iceclimbers', 'sora', 'banjo']},
...
function populateSlots() {
    $.each(slots, function (slot, item) {
        ...
        $.each(item.next, function (index, character) {
            var width, height, ownLeft, ownTop, iconX, iconY, textX, textY;
            switch (direction) {
                case 'right_left':
            ...
            var isProperty = !['chest', 'chance', 'banned', 'free', 'tax'].includes(type);
            var icon = new Image();
    ...
    $.each(slots, function(index, item){
        ...
        switch (item.direction){
            ...
            case 'bottom_top':
                ...
                offsetY += item.hotel ? 50 :60;
                offsetX += item.hotel ? 0 : 5;
                break;
            case 'left_right':
                rotateBy = Math.PI;
                ctx.textAlign = 'right';
        ...
        if (item.icon){
            ...
            if (['chest', 'chance', 'tax', 'utility', 'station'].includes(item.type)){
                ...
                x = item.ownLeft,y=item.ownTop;w=standardHeight;h=standardHeight;
            } else {
            ...
            if (addText){
                ...
                ctx.font = '15px "Futura PT Medium"';
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.fillStyle = 'black';
                ctx.fillText(item.name.toUpperCase(), 0, -50);
            } else {
                ctx.drawImage(item.icon, 0, 0, w, h);
            }
        ...
        if (item.owner || item.owner === 0) {
            $('.player[data-number="'+item.owner+'"] .owned_characters').append('<span style="width:100%;border:2px solid black;color:white;background-color:'+characterColour+'">'+item.name+'</span><br />');
            // Translucent rectangle over whole spot
            ctx.fillStyle = 'rgba(0, 0, 255, 0.2)';
            ctx.fillRect(item.left, item.top, item.right-item.left, item.bottom-item.top);

            ctx.drawImage(players[item.owner].image, item.ownLeft + 5, item.ownTop + 5, 20, 20);
            if (item.hotel) {
                ctx.drawImage(hotel, item.ownLeft + offsetX, item.ownTop + offsetY, 30, 30);
            } else if (item.house) {
                ctx.drawImage(house, item.ownLeft + offsetX, item.ownTop + offsetY, 20, 20);
            }
        }
    });
}
